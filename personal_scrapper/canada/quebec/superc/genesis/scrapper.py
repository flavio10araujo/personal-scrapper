...
def print_category_tree(categories, level=0):
    ...
    for cat_group in categories:
        for cat in cat_group:
            print(f"{indent}- {cat['Category']}")
            if cat['Subcategories']:
                print_category_tree([cat['Subcategories']], level + 1)

def scrape_all_categories():
    global seen_skus
    seen_skus = set()

    categories_structure = get_categories_structure()
    print("Category Tree:")
    print_category_tree(categories_structure)
